...
import { useEditBrand, useGetBrand } from "@/src/hooks/brand.hook";
...
const UpdateBrandForm = ({ id }: { id: string }) => {
    const {
        register,
        handleSubmit,
        reset,
    ...
    const { mutate: handleEditBrand, isLoading, isSuccess } = useEditBrand()
    ...
    const onSubmit = async (data: FieldValues) => {
        ...
        handleEditBrand({ id, data: formData })
    };

    useEffect(() => {
        if (isSuccess) {
            Toast("success", "Edited brand successfully!")
        ...
        <div className="flex justify-center px-8 mt-10 bg-gray-100">
            <div className="bg-white rounded-lg shadow-lg w-full">
                <div className=" mb-4 border-b py-4 flex items-center justify-between px-6">
                    <h2 className=" text-xl font-semibold text-start">Edit Brand</h2>
                    <Link href={"/dashboard/admin/brand"} className="w-fit py-2 px-4 font-medium bg-orange-500 text-white rounded-md shadow-sm hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2">
                        Manage Brands
                    </Link>
                </div>
